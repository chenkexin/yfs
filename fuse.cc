...
{
    std::cout<<"----------------------in fuseserver_getattr"<<endl;
    struct stat st;
    ...
    std::cout<<"----------------------end fuseserver_getattr"<<endl;
...
{
    std::cout<<"-----------------------------------fuseserver_setattr"<<endl;
        /*
        ...
        std::cout<<"in fuseserver_setattr, which is the size:????: to_set "<<to_set<<" or"<<attr->st_ino<<endl;
    printf("fuseserver_setattr 0x%x\n", to_set);
    if (FUSE_SET_ATTR_SIZE & to_set) {
        printf("   fuseserver_setattr set size to %zu\n", attr->st_size);
        std::cout<<"the ino in attr is:"<<attr->st_ino<<endl;
        yfs->setattr( ino, attr->st_ino );
        getattr( ino, *attr );
    ...
    fuse_reply_err(req, ENOSYS);
        ...
        // The operating system may implement overwriting an existing file with a call to SETATTR (to truncate the file) rather than CREATE. Setting the size attribute of a file can correspond to truncating it completely to zero bytes, truncating it to a subset of its current length, or even padding bytes on to the file to make it bigger. Your system should handle all these cases correctly.
}
...
fuseserver_write(fuse_req_t req, fuse_ino_t ino,
        ...
        struct fuse_file_info *fi)
{
    std::cout<<"---------------------in fuseserver_write"<<endl;
    /*
     * your lab2 code goes here.
    ...
    std::cout<<endl;
    yfs->write( ino, size, off, buf, bytes_written);
    fuse_reply_write(req, bytes_written); 
    std::cout<<"---------------------end fuseserver_write"<<endl;
#else
    fuse_reply_err(req, ENOSYS);
#endif
}
        ...
        mode_t mode, struct fuse_entry_param *e, uint32_t type)
{
    ...
    if( ret == yfs_client::OK )
    {
        ...
        e->ino = ino_out;
        //the step that put the parent's context has been done in yfs->create
        //change the parent's ctime and mtime is done in write_block()
        std::cout<<"---------------------return createhelper OK"<<endl;
        return yfs_client::OK;
    }
    else if( ret == yfs_client::EXIST)
    {
        getattr( ino_out, e->attr);
        e->ino = ino_out;
        std::cout<<"---------------------return createhelper not OK"<<endl;
...
{
    std::cout<<"---------------------------in fuseserver_create"<<endl;
    struct fuse_entry_param e;
    yfs_client::status ret = fuseserver_createhelper( parent, name, mode, &e, extent_protocol::T_FILE);
    if( ret  == yfs_client::OK ) 
    {
...
void fuseserver_mknod( fuse_req_t req, fuse_ino_t parent, 
    ...
    std::cout<<"------------------------in fuseserver_mknod"<<endl;
    struct fuse_entry_param e;
    yfs_client::status ret;
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
    ...
    std::cout<<"------------------------in fuseserver_lookup"<<endl;
    bool found = false;
    ...
    {
        std::cout<<"lookup not ok."<<endl;
        fuse_reply_err(req, 0);
    }
    if( found == true && ret == yfs_client::EXIST )
    {
        getattr( inum, e.attr);
        e.ino = inum;
        fuse_reply_entry(req, &e);
        ...
        off_t off, struct fuse_file_info *fi)
{
    yfs_client::inum inum = ino; // req->in.h.nodeid;
    struct dirbuf b;
    std::cout<<"-----------------in fuseserver_readdir"<<endl;
    ...
    std::cout<<"in fuse_readdir, require lock:"<<inum<<endl;
    yfs->lc->acquire(inum);
    std::cout<<"holding lock:"<<inum<<endl;
    yfs->readdir( inum, list ); 
    yfs->lc->release(inum);
    std::cout<<"already releasing lock:"<<inum<<endl;
    std::list<yfs_client::dirent>::iterator iter;    
    for( iter = list.begin(); iter != list.end(); iter++)
    {
        dirbuf_add( &b, iter->name.c_str(), iter->inum );    
    }
    std::cout<<"--------------reply fuseserver_readdir"<<endl;
